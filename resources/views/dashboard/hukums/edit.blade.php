<x-dashboard-layout>
    <x-slot:title>Ubah Informasi Lembaga</x-slot:title>
    <h1 class="text-5xl mb-2 font-extrabold ml-8">Ubah Informasi Lembaga</h1>
    <p class="mb-8 ml-8">Halaman ini digunakan untuk menambahkan jabatan baru</p>

    <form method="POST" action="/dashboard/lembagas/{{ $lembaga->slug }}" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        ...
        <div class="space-y-6 mr-4 mb-10 ml-8">
            ...
            <div>
                ...
                <input type="hidden" id="oldImage" name="oldImage" value="{{ $lembaga->image }}">
                <div class="flex flex-col sm:flex-row items-center justify-center w-full space-x-4 space-y-4">
                    ...
                    <div id="preview-container" class="w-full sm:w-1/2 mb-4 sm:mb-0">
                        @if ($lembaga->image)
                            <img src="{{ asset('storage/' .  $lembaga->image ) }}" alt="">
                        @endif
                    </div>
                </div>
                @error('image')
